...
sum <- function(x) {
    output = 0
    for (thing in x) {
        output = as.integer(output) + as.integer(thing)
    }
    return(output)
}
...
# Essentially, 100% of the chart is the non local population, each slice is the continent of origin
...
# Getting the total nonlocal population
nonlocal = sensus(pruned, hush = TRUE)
mini = groups(pruned, hush = TRUE)

# Making the ratios
percent = c()
for (entry in 1:length(mini)) {
    relative = mini[entry]/nonlocal
    percent[entry] = relative
}

pie(percent, colnames(pruned)[2:ncol(pruned)])
